...
Usage:
    tradememory demo          # with typewriter effect
    tradememory demo --fast   # skip delays
...
def main(fast=False):
    ...
    print_header("TradeMemory Protocol — Interactive Demo", fast=fast)
    slow_print("  Simulating 30 XAUUSD trades over 7 days.", fast=fast)
    ...
    # ──────────────────────────────────────────────────
    print_step(1, "L1 — Recording trades to TradeJournal", fast=fast)
    slow_print("  Every trade decision is stored with full context:", fast=fast)
    ...
    try:
        ...
        if sample:
            ...
            print()
            slow_print("  Full TDR JSON available via: GET /audit/decision-record/{trade_id}", fast=fast)
            slow_print("  Batch export: GET /audit/export?start=2026-03-01&end=2026-04-01", fast=fast)
            slow_print("  Verify hash:  GET /audit/verify/{trade_id}", fast=fast)
    ...
    slow_print("  that analyzes YOUR actual trades with deeper insights.", fast=fast)
    print()
    print("  Next steps:")
    print("    1. tradememory setup")
    print("    2. Add your ANTHROPIC_API_KEY")
    print("    3. tradememory doctor --full")
    print()
