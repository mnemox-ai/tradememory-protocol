...
## Recent Changes
- [2026-03-27] docs: README 重寫 — audit + memory 雙核心定位。新增 Decision Audit Trail section（真實 JSON）、Regulatory Alignment table、Security section、Audit MCP tools。badges 更新 1,233 tests / 17 tools / 35+ endpoints。
- [2026-03-27] release: v0.5.1 bump — audit endpoints + pipeline fixes + Binance Skills Hub SKILL.md。等 PyPI token 發版。
- [2026-03-27] fix: pipeline P0-P2 — strategy_map 直接匹配（不再 map 到 NG_Gold）、TDR builder 讀 JSONL indicators、concurrent write corruption handler。1233 tests。
...
## Current Status
- **v0.5.1** (pending PyPI publish), 1233 tests passing, 2 skipped
...
- 17 MCP tools, 35+ REST endpoints, Evolution Engine + OWM 5 memory types
